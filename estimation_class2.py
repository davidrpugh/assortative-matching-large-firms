...
class HTWF_Estimation(object):
	...
	'''
	def __init__(self, x_pam, x_bounds, y_pam, y_bounds, x_scaling,A_scaling,knots):
		'''
	...
	def Solve_Model(self, F_params, N_knots, intg, ini, tolerance):
		...
		''' 1.Solve the Model '''
		solver.solve(ini, tol=tolerance, number_knots=N_knots, knots=self.knots, integrator=intg)
	...
	def Calculate_MSE(self, functions_from_model,penalty=100):
		...
		dummy:					Make True if dummy data from model simulation	
		...
		cdf_theta_data = np.cumsum(weights)/np.sum(weights) #there was a mistake here, frequency is just cumulative sum of weights, there should be no theta in here!
	...
	def StubbornObjectiveFunction(self, params, N_points, tol_i, guess):
		"""
		...
		# 3. Solve the model
		try:
			sol = self.Solve_Model(F_params, N_points, 'lsoda', guess, tol_i)		
		except AssertionError ,e:
			if e=='Failure! Need to increase initial guess for upper bound on firm size!':
				guess = guess * 10
				print guess
			try: 
				sol = self.Solve_Model(F_params, N_points, 'vode', guess, tol_i)
			except AssertionError:
				try: 
					sol = self.Solve_Model(F_params, N_points, 'lsoda', guess*100.0, tol_i)					 
				except AssertionError:
					try: 
						sol = self.Solve_Model(F_params, N_points, 'lsoda', guess/100.0, tol_i)
					except AssertionError:
						try:
							sol = self.Solve_Model(F_params, N_points,  'lsoda', guess, tol_i*10)
						except AssertionError:
							if N_points is not None:
								try: 
									sol = self.Solve_Model(F_params, N_points*1.5, 'lsoda', guess, tol_i)
								except AssertionError, e:
	...
	def Plot_data(self):
		...
		cdf_theta_data = np.cumsum(weights)/np.sum(weights)
