...
plt.savefig('graphics/multiplicative-separability-figure3a.pdf')
plt.show()
...
for i, val in enumerate(sigma_As):
    ...
    # generate the plots
    ax.plot(tmp_data['$\\mu(x)$'], tmp_data['$\\theta(x)$'], color=colors[i], 
            label=r'$\sigma_A=%.2f$' %val)
    ax.plot(tmp_data.index, tmp_data['$\\mu(x)$'], color=colors[i], 
            label=r'$\sigma_A=%.2f$' %val)
    ax.plot(tmp_data['$\\mu(x)$'], tmp_data['Profits, $\\pi(\\mu(x))$'], 
            color=colors[i], label=r'$\sigma_A=%.2f$' %val)
    ax.plot(tmp_data.index, tmp_data['Wages, $w(x)$'], color=colors[i], 
            label=r'$\sigma_A=%.2f$' %val)
...
plt.savefig('graphics/multiplicative-separability-figure3b.pdf')
plt.show()
