...
class Model(object):
    ...
    def output(self, x, y, l, r):
        """
        Wrapped extensive form of the production technology. Necessary as Sympy
        function doesn't allow one to specify extra args like params.
        ...
        The first two arguments x and y are quality variables describing the 
        worker and firm types, while the latter two aruments are quantity 
        variables describing the level of inputs.

        """
        return self.F(x, y, l, r, self.params)
        
    def intensive_output(self, x, y, theta):
        """
        Intensive form of the production technology. 

        Arguments:

            x:      worker type
            y:      firm type
            theta:  factor intensity (i.e., (l / r))

        """
        return self.output(x, y, theta, 1)

    def compute_partial_derivatives(self):
        """
        Computes symbolic partial and cross-partial derivatives of the
        production function F and its intensive form, f.

        """
        # define symbolic vars
        x, y, l, r, theta = sp.symbols('x,y,l,r,theta')

        # Marginal product of hiring a better worker
        tmp_Fx = sp.diff(self.output(x, y, l, r), x, 1)
        Fx     = sp.lambdify((x, y, l, r), tmp_Fx)
        
        # Marginal product of better firm
        tmp_Fy = sp.diff(self.output(x, y, l, r), y, 1)
        Fy     = sp.lambdify((x, y, l, r), tmp_Fy)
         
        # Marginal product of hiring hiring an extra worker of type x
        tmp_Fl = sp.diff(self.output(x, y, l, r), l, 1)
        Fl     = sp.lambdify((x, y, l, r), tmp_Fl)
        
        # Marginal product of devoting exta unit of resources to a worker
        tmp_Fr = sp.diff(self.output(x, y, l, r), r, 1)
        Fr     = sp.lambdify((x, y, l, r), tmp_Fr)
        
        # Type complementarity
        tmp_Fxy = sp.diff(self.output(x, y, l, r), x, 1, y, 1)
        Fxy     = sp.lambdify((x, y, l, r), tmp_Fxy)
        
        # Span of control complementarity
        tmp_Fyl = sp.diff(self.output(x, y, l, r), y, 1, l, 1)
        Fyl     = sp.lambdify((x, y, l, r), tmp_Fyl)
        
        # Managerial resource complementarity-1
        tmp_Fxr = sp.diff(self.output(x, y, l, r), x, 1, r, 1)
        Fxr     = sp.lambdify((x, y, l, r), tmp_Fxr)
        
        # Managerial resource complementarity-2
        tmp_Flr = sp.diff(self.output(x, y, l, r), l, 1, r, 1)
        Flr     = sp.lambdify((x, y, l, r), tmp_Flr)
        
        # Used to compute the real wage
        tmp_ftheta = sp.diff(self.intensive_output(x, y, theta), theta, 1)
        ftheta     = sp.lambdify((x, y, theta), tmp_ftheta)
    ...
    def update_parameter(self, param, new_value):
        """
        Updates the value of a specified model parameter, re-wraps the output
        function, and re-computes the dictionary of partial and cross-partial
        derivatives. 
        
        Arguments:
            
            param:     (str) Name of the model parameter you wish to update.
        ...
        # re-compute partial and cross-partial derivatives
        self.derivatives_dict = self.compute_partial_derivatives()
        
    ##### Methods for describing the economic equilibrium #####
    ...
    def H(self, xn, vec):
        ...
        """
        return self.workers.scaled_pdf(xn) / self.firms.scaled_pdf(vec[1])

    def get_wages(self, xn, vec):
        """
        ...
        Returns:
            ...
            real_wage: (array) Array of values defining the real wage paid to a
                       worker with skill level x.
                     
        """
        # unpack the vector of endogenous variables
        theta, mu = vec
        
        # workers are paid there marginal products
        real_wage = self.derivatives_dict['ftheta'](xn, mu, theta)
        
        return real_wage

    def get_profits(self, x, vec):
        """
        Equilibrium profits of a firm matched with a worker of type x.
        ...
        Returns:
            ...
            profits: (array) Array of values defining firm profits as a function
                     of the skill of a worker it employs.
        ...
        revenue = self.intensive_output(x, mu, theta) 
        costs   = theta * self.get_wages(x, vec) 

        profits = revenue - costs

        return profits
    
    def theta_prime(self, x, vec, matching='pam'):
        """
        ...
        Returns:
            ...
            RHS: (array) RHS of the differential equation governing the 
                 evolution of factor intensity.

        """
        # unpack the vector of endogenous variables
        theta, mu = vec
        
        # extract the cross partial derivatives
        Fyl = self.derivatives_dict['Fyl'](x, mu, theta, 1)
        Fxr = self.derivatives_dict['Fxr'](x, mu, theta, 1)
        Flr = self.derivatives_dict['Flr'](x, mu, theta, 1)

        # compute theta_prime
        if matching == 'pam':
            RHS = (self.H(x, [theta, mu]) * Fyl - Fxr) / Flr      
        elif matching == 'nam':
            RHS = -((self.H(x, [theta, mu]) * Fyl + Fxr) / Flr)
        else:
            raise ValueError          
    ...
    def mu_prime(self, x, vec, matching='pam'):
        """
        ODE governing the evolution of matching process.
